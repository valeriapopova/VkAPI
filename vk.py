...
def ads_get_statistic(vk, account_id=None, ids=None):
    ...
    result = vk.method('ads.getStatistics', {'account_id': account_id, 'period': 'day',
                    'date_from': '2021-09-08', 'date_to': '2021-09-08', 'ids_type': 'ad', 'ids': ids, 'v': VK_VERSION})
    r = result.json()
    error = r.get('error')
    if error:
...
def get_demographics(vk, account_id=None, ids=None):
    ...
    result = vk.method('ads.getDemographics', {'account_id': account_id, 'ids_type': 'ad', 'ids': ids,
                                               'period': 'day',
                                               'date_from': '2021-09-08', 'date_to': '2021-09-08',
                                               'v': VK_VERSION})
    r = result.json()
    error = r.get('error')
    if error:
